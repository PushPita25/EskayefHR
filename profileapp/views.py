...
#             subject = f'RRF_{recruitment_form_instance.department}_{recruitment_form_instance.vacant_position} Position'

#             email_body = (
#                 f"RRF_{recruitment_form_instance.department}_{recruitment_form_instance.designation}_NoOfVacantPosition:{recruitment_form_instance.vacant_position} has been submitted"
#             )
...
#             html_string = render_to_string('profileapp/expense_pdf_template.html', {'expense': expense_instance})
#             pdf_file = HTML(string=html_string).write_pdf()
#             pdf_filename = f'expense_report_{expense_instance.id}.pdf'
            
#             # Store PDF in session or temporary storage
#             request.session['pdf_file_path'] = save_pdf_temporarily(pdf_file, pdf_filename)
            
#             # Success message after form submission
#             messages.success(request, 'Your Expense Report is submitted successfully. You can download it now.')
            
#             return redirect('dashboard')  # Redirect to the dashboard or a confirmation page

#     else:
#         form = handle_get_request(request)
...
# def save_pdf_temporarily(pdf_data, filename):
#     # You would implement actual file saving here and return the path
#     path = '/Users/zarintasnim/Desktop/MyProject_old/Profile/profileapp/pdfs' + filename
...
#         'location': row[4] if row else '',
#     }
#     return ExpenseForm(initial=initial_data)
